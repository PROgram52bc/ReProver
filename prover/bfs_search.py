"""Proof search using breadth-first search.
"""
...
class BreadthFirstSearchProver:
    """A prover that uses breadth-first search to find proofs using a tactic generator."""
    ...
    def search(
        ...
        logger.info(f"Proving {thm} using BFS")
        ...
        try:
            with Dojo(thm, self.timeout, additional_imports=imps) as (
                ...
                try:
                    asyncio.run(self._breadth_first_search())
    ...
    async def _breadth_first_search(self) -> None:
    ...
    async def _step(self, queue):
        ...
        for tactic, logprob in suggestions:
            edge, finished = self._run_tactic(
                search_node, tactic, logprob, queue
            )
            results.append(edge)
            if finished:
                break
    ...
    def _run_tactic(
        self, node: InternalNode, tactic: str, logprob: float, queue
    ) -> Tuple[Edge, bool]:
        t0 = time.time()
